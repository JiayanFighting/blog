...
let defaultText = `
<img src="https://weekly.omsz.io:3000/7/jiayan.huang@dchdc.net/A05B71F8968B455EA07817D28EB9EA61.png" alt=“upload”  width="50%" height="50%">
...
class WriteReport extends Component {
  ...
  componentDidMount() {
    ...
    if (
      this.props.currReport &&
      Object.keys(this.props.currReport).length > 0
    ) {
  ...
  handleAddSprint = (values) => {
    let type =
      values.type === undefined ? this.state.sprintObj.type : values.type;
    let sprint =
      values.sprint === undefined
        ? this.state.sprintObj.sprint + 1
        : values.sprint;
    let beginTime;
    let endTime;
    if (values.date === undefined) {
      //defalut
      beginTime = moment(this.state.sprintObj.endTime, "YYYY-MM-DD")
        .add(1, "days")
        .format("YYYY-MM-DD");
      endTime = moment(this.state.sprintObj.endTime, "YYYY-MM-DD")
        .add(7, "days")
        .format("YYYY-MM-DD");
      if (type === "daily") {
        beginTime = moment(this.state.sprintObj.endTime, "YYYY-MM-DD")
        ...
        beginTime = moment(this.state.sprintObj.endTime, "YYYY-MM-DD").add(
          1,
          "days"
        );
        endTime = moment(this.state.sprintObj.endTime, "YYYY-MM-DD").add(
          30,
          "days"
        );
      }
    ...
    createSprintItemService(sprintItem)
      .then((res) => {
        if (res.code === 0) {
  ...
  onShowReportsInThePastClicked = () => {
    ...
    // }
    var params = {
      fromEmail: this.state.userEmail, // opt , if no userEmail ,required
    ...
    return (
      <Layout>
        ...
        <div style={!infoPage ? {} : { display: "none" }}>
          ...
          {this.state.toTeam && this.state.sprint !== 0 ? (
            <WriteAndViewBoard3
            ...
            <Spin tip="Loading..." spinning={this.state.isLoading}>
              ...
              >
                ...
                <Form.Item
                  ...
                  initialValue={
                    this.state.toTeam ? this.state.toTeam.ccList : ""
                  }
                >
              ...
              <Col span={24}>
                ...
                >
                  <Form.Item
                    ...
                    initialValue={
                      this.state.sprintObj ? this.state.sprintObj.sprint + 1 : 1
                    }
                  >
                    <InputNumber />
                  </Form.Item>
